...
#define HIST_SIZE 100   // Warning: direct impact on RAM overflow during compilation
...
******************************************************************************/
Tennis::Tennis(Pinetime::Components::LittleVgl& lvgl) : lvgl {lvgl} {
  menuItem = 0;
  appStep = 0;
  headHist = 0;
  memHist = 0;
...
void DisplayScore( lv_obj_t *Scr, int state ) {
  if ( state == 0 ) {
    ...
    lv_obj_align( scoreT, Scr, LV_ALIGN_CENTER, 10, -70 );

    // Game score Bottom
    scoreB = lv_label_create( Scr, nullptr );
    lv_obj_add_style( scoreB, LV_OBJ_PART_MAIN, &score_st );
    lv_obj_align( scoreB, Scr, LV_ALIGN_CENTER, 10, 70 );
    ...
    if (headHist == memHist)
      ...
      lv_obj_set_style_local_bg_color(imgBall, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_MAROON);

    // Indicate server position (Top or Bottom)
    if (Result[headHist].server == 0)
      lv_obj_align( imgBall, Scr, LV_ALIGN_IN_LEFT_MID, 20, -80 );
    else
      lv_obj_align( imgBall, Scr, LV_ALIGN_IN_LEFT_MID, 20, 80 );
...
void InitHistory( void )
{
  ...
  tailHist = 0;
  memHist = 0;
//GDV  Result[headHist].format = matchFormat;
  format = matchFormat;
...
//GDV    Result[headHist].format = Result[prevStep].format;
...
void ManageSet( int G, int P, int max, int diff ) {
  ...
  if ((Result[headHist].sets[G][Result[headHist].set] == (max + 1)) and Result[headHist].sets[P][Result[headHist].set] == max) {
    Result[headHist].gainSets[G] +=1;
    if (Result[headHist].gainSets[G] > maxSets/2) {
      exitScore = true;
      matchSetType[Result[headHist].set] = 0;
